	...
	$.ajax({
	    ...
	    success:function(data,textStatus,jqXHR){
	        ...
	        jQuery.each(data.list,function(i,item){
	        	...
	        	if(item.state == 0){
	        		content = content + "<B style='color:red'>(该内容已被屏蔽)</B>";
...
//屏蔽回复贴
function shieldReplyPost(replyPostId,state){
	if (confirm("确定要屏蔽该回复吗？")) {
		var url = "/guestbook/rs/replypost/admin/updateState";
		var obj = {
